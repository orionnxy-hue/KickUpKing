...
const App: React.FC = () => {
    ...
    const addNotification = useCallback((title: string, message: string, type: 'success' | 'info' | 'error' = 'info') => {
        ...
        setNotifications(prev => [{ id, title, message, type, timestamp: Date.now(), read: false }, ...prev].slice(0, 5));
    }, []);

    const syncCloudError = useCallback(() => {
    ...
    // ... (Keep existing effects for Online Presence, Timer etc. - functionality unchanged)
    ...
    const handleGameOver = useCallback(async () => {
        ...
        setQuests(newQuests);
        if (userId) {
            dbService.updateScore(userId, score, tokens, currentBest, inventory, playerLevels, friends)
                .then(() => syncCloudError())
                .catch(() => syncCloudError());
        }
    ...
    // ... (Remaining handlers for RejectRequest, RemoveFriend, ClearNotification are standard state updates)
